...
async def list_clientes(
    ...
    result = await db.execute(q.order_by(Cliente.apellido))
    return result.scalars().all()


@router.get("/{id}", response_model=ClienteResponse)
async def get_cliente(id: int, db: AsyncSession = Depends(get_db), _: str = Depends(verify_token)):
...
async def delete_cliente(id: int, db: AsyncSession = Depends(get_db), _: str = Depends(verify_token)):
    c = await db.get(Cliente, id)
    if not c:
        raise HTTPException(status_code=404, detail="Cliente no encontrado")
    ventas_count = await db.scalar(select(func.count()).where(Venta.cliente_id == id))
    if ventas_count:
