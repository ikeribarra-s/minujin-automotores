...
async def get_permuta(id: int, db: AsyncSession = Depends(get_db), _: str = Depends(verify_token)):
...
async def create_permuta(
    ...
    vehiculo = await db.get(Vehiculo, data.vehiculo_recibido_id)
    if not vehiculo:
        raise HTTPException(status_code=404, detail="El vehículo recibido no existe en el sistema")

    permuta = Permuta(**data.model_dump())
    db.add(permuta)
    await db.commit()
    await db.refresh(permuta)
    return permuta
